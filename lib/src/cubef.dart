...
class CubefState extends State<Cubef> with SingleTickerProviderStateMixin {
  ...
  late List<Widget> _verticalIndex;
  late List<Widget> _horizontalIndex; // initial index of the cube side
  ...
  void initState() {
    ...
    if (widget.controller == null) {
      _controller = AnimationController(vsync: this, duration: Duration(milliseconds: widget.animationDuration));
    } else {
    ...
    _animation = _tween.animate(CurvedAnimation(parent: _controller, curve: (widget.animationEffect == null) ? Curves.decelerate : widget.animationEffect!));
    ...
    _controller.addListener(() {
      setState(() {
        ...
        if ((_tween.end! >= _tween.begin! && _animation.value >= _tween.end! - 0.2) || (_tween.end! < _tween.begin! && _animation.value <= _tween.end! + 0.2)) {
          if (_lastAction == Action.down || _lastAction == Action.up)
            initStackChildrenTop(_verticalIndex[0]);
          else if (_lastAction == Action.left || _lastAction == Action.right) initStackChildrenTop(_horizontalIndex[0]);
        }
      });
    });
